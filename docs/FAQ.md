...
Fresh Termux ka curl tab tak toota hai jab tak base packages upgrade na hon — aur `pkg` khud usi curl se mirror chunta hai, isliye `pkg upgrade` bhi wahi error deta hai (Moto pe dono dekhe). Isliye command `apt update && apt full-upgrade` se shuru hoti hai (apt curl nahi use karta). Ab wahi line mirror bhi khud set karti hai (Termux ka Cloudflare mirror) agar koi set nahi tha — neela `termux-change-repo` dialog dekhna hi nahi padta. Phir bhi mirror error aaye to `termux-change-repo` (Enter, Enter) aur wahi line dobara.
...
**`hello` likha to "kisi brain ne jawab nahi diya" — install to ho gaya tha?**
Install theek hai; **dimaag** (brain) nahi laga. Do raaste, dono `ai` ke pehle start pe card me likhe aate hain: (1) free cloud key 2 min me — `setup-menu` → **G** (guided; Groq/Gemini free tier); (2) local brain — wizard me chuna tha to `setup-menu → brain` (Ollama + model download, GBs). `ai doctor` bataata hai abhi kya hai aur kya nahi. Bina dimaag ke bhi ye chalta hai: `= 2+2`, `date`, `battery`, `/hands`, `/list`, `/remind`, `ai tour`.

**Terminal ke colours badal sakta hai? Meri settings to nahi chhedega?**
Installer ki aakhri stage (Look + shortcut) poochhti hai — Enter = aasmaan theme, ya 2/3/4, ya n. Font nahi badalta: Termux me size pinch se, family `~/.termux/font.ttf` — tera hai, hum nahi chhedte. `/theme` — chaar palettes (light, dark, nerd, aasmaan), colour theory se, contrast code me maapa hua (text ≥ 7:1, accents ≥ 3:1). Bina `save` ke sirf is session ke colours badalte hain (OSC sequences — koi file nahi chhuti), aur har start pe dobara lag jaate hain. `/theme aasmaan save` tab hi tere terminal ki config likhta hai — ek backup pehle (`~/.ai-theme-backup/`), `/theme undo` wapas. `/theme off` = terminal ke apne default. Uninstall bhi pehle undo chalata hai.
