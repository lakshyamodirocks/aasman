...
if [ "${1:-}" = "--uninstall" ]; then
  ...
  RM=( "$HOME/.local/bin/ai" "$HOME/.local/bin/VERSION" "$HOME/.local/bin/setup-menu" "$HOME/.local/bin/screen-dump" "$HOME/.local/bin/vault-backup"
       ...
       "$HOME/.ai-first-run" "$HOME/.ai-shortcuts.json" "$HOME/.shortcuts/Aasmaan" "$HOME/.shortcuts/icons/Aasmaan.png" "$HOME/.ai-theme.json" "$HOME/.ai-theme-backup" "$HOME/.ai-term.json" "$HOME/.ai-reminders.json" "$HOME/.ai-greet.json" "$HOME/.ai-lists.json" "$HOME/.ai-tuning.json" "$HOME/.ai-usage.json" "$HOME/.ai-hands.json" "$HOME/.ai-device.json" "$HOME/.ai-chat.json" "$HOME/.ai-cache.jsonl" "$HOME/.ai-metrics.json" "$HOME/.ai-kb.jsonl" "$HOME/.ai-brains.json" "$HOME/.ai-daemon.json" "$HOME/.ai-update.json" "$HOME/.ai-egress.log" "$HOME/.ai-first-cloud" "$HOME/.ai-telegram.json"
