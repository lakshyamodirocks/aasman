...
```bash
apt update && apt -y -o Dpkg::Options::=--force-confnew full-upgrade && apt -y install curl python && curl -fsSL https://raw.githubusercontent.com/lakshyamodirocks/aasman/main/install.sh | bash
```
The upgrade first is not optional, and it is `apt`, not `pkg`: a fresh Termux ships a `curl` that fails with a libcurl/SSL symbol error until the base packages are upgraded, and `pkg` itself calls that broken curl to pick a mirror, so `pkg upgrade` dies the same way (both seen on a Moto, 2026-09-06). `apt` fetches with its own code. If `apt update` stops with a *mirror / unable to resolve* error, run `termux-change-repo` (Enter, Enter) and paste the command again.
