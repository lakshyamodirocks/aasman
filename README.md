...
- **Runs and is gated on Linux**: a gate of scripted installs from this exact bundle, uninstall, and a golden set of 144 pinned behaviours (`tests/golden.py`) and an adversarial set of 10 trust invariants proven by attack (`tests/adversarial.py`). CI repeats golden + adversarial + install (see `.github/workflows/ci.yml`) on Linux, macOS and Windows; the badge at the top is the live answer.
